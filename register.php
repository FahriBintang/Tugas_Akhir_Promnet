<?php 
require 'function.php';

$error = "";
$success = ""; // Inisialisasi agar tidak error "Undefined variable"

if (isset($_POST["tombol_register"])) {
    $hasil = register($_POST);

    if ($hasil === 1) {
        // Tanda berhasil menggunakan alert browser
        echo "<script>
                alert('Pendaftaran Berhasil! Akun Anda telah terdaftar.');
                document.location.href = 'login.php';
              </script>";
        exit;
    } else {
        // Jika gagal, simpan pesan error string dari function register
        $error = $hasil;
    }
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Register Page</title>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background: #f6f7fb; }
        .login-card {
            margin-top: 15%; /* Sedikit dikurangi agar tidak terlalu ke bawah */
            padding: 30px;
            border-radius: 10px;
            background: white;
            box-shadow: 0 4px 18px rgba(0,0,0,0.1);
        }
    </style>
</head>
<body class="class="fw-bold fs-4 text-black style="font-family: 'Poppins';">

<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-4">
            <div class="login-card">
                <h3 class="text-center mb-4">Register</h3>

                <?php if($error) : ?>
                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                    <?= $error ?>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
                <?php endif; ?>

                <form action="" method="POST">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Username</label>
                        <input type="text" name="username" class="form-control" placeholder="Masukkan username..." autocomplete="off" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Email</label>
                        <input type="email" name="email" class="form-control" placeholder="Masukkan email..." autocomplete="off" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Password</label>
                        <input type="password" name="password" class="form-control" placeholder="Masukkan password..." autocomplete="off" required>
                    </div>

                    <div class="mb-3">
                        <label for="kode_admin" class="form-label fw-bold">Kode Admin (Opsional)</label>
                        <input type="text" class="form-control" id="kode_admin" name="kode_admin" placeholder="Masukkan kode jika Anda admin...">
                    </div>

                    <button type="submit" name="tombol_register" class="btn btn-warning w-100">Register</button>
                    <p class="mt-2 text-center">Sudah punya akun? <a href="login.php">Login</a></p>
                </form>
            </div>
        </div>
    </div>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>